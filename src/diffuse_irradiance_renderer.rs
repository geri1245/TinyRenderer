...
use wgpu::{CommandEncoder, Device, ImageDataLayout, SubmissionIndex, TextureFormat};
...
use crate::{
    ...
    pipelines::DiffuseIrradianceBakerRP,
};
...
impl DiffuseIrradianceRenderer {
    pub async fn new(
        ...
        queue.write_texture(
            ...
            ImageDataLayout {
                offset: 0,
                bytes_per_row: Some(CUBEMAP_RESOLUTION * 4 * 2), // 4 components, 2 bytes per component (rgba16)
                rows_per_image: Some(CUBEMAP_RESOLUTION),
            },
            size,
        );
        ...
        let output_buffer = OutputBuffer::new(device, CUBEMAP_RESOLUTION, CUBEMAP_RESOLUTION);

        Ok(Self {
            pipeline,
            mesh: basic_mesh,
            render_params: render_into_cubemap_params,
    ...
    pub fn render(&self, encoder: &mut CommandEncoder, hdr_environment_cube_map: &wgpu::BindGroup) {
        ...
        encoder.copy_texture_to_buffer(
            self.ibl_irradiance_texture.as_image_copy(),
            wgpu::ImageCopyBuffer {
                buffer: &self.output_buffer.buffer,
                layout: wgpu::ImageDataLayout {
                    offset: 0,
                    bytes_per_row: Some(self.output_buffer.get_bytes_per_row()),
                    rows_per_image: Some(CUBEMAP_RESOLUTION),
                },
            },
    ...
    pub fn write_current_ibl_to_file(&self, device: &Device, submission_index: SubmissionIndex) {
